...
class SelectionContext {
    ...
    getSentence() {
        ...
        log( 'getSentence', [ this.outputIteration, context.context, this.node ] );

        return this.getSentence();
    }

    __cutSentence( word, index, text ) {
        log( '__cutSentence', arguments );
        ...
        log( '__cutSentence', [ left, right, text ] );
    ...
    __cutBeforeSelection( text ) {
        this.sentenceDeviderMarks.forEach( ( mark ) => {
            ...
            log( '__cutBeforeSelection', [ text, index, text.slice( index + 1 ) ] );
